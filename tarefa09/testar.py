...
class BaseTask:
    ...
    def _filter_output(
        self,
        text: str,
        keep_contains: list[str] | None = None,
        keep_regexes: list[str] | None = None,
        drop_contains: list[str] | None = None,
        drop_regexes: list[str] | None = None,
        *,
        fuzzy_keep: bool = True,
        fuzzy_threshold: float = 0.72,
        fallback_to_all_on_empty: bool = True,
    ) -> str:
        
        keep_contains = [self._normalize_for_match(kc) for kc in (keep_contains or [])]
        drop_contains = [self._normalize_for_match(dc) for dc in (drop_contains or [])]
        keep_re = [re.compile(p, flags=re.I) for p in (keep_regexes or [])]
        drop_re = [re.compile(p, flags=re.I) for p in (drop_regexes or [])]
        ...
        for raw in lines:
            ...
            if any(norm.startswith(dc) for dc in drop_contains):
                continue
            if any(p.search(raw) for p in drop_re):
                continue

            keep_ok = not (keep_contains or keep_re)

            if not keep_ok and keep_contains:
                if any(kc in norm for kc in keep_contains):
                    keep_ok = True
                elif fuzzy_keep:
                    # fuzzy contra cada kc
                    for kc in keep_contains:
                        if difflib.SequenceMatcher(None, norm, kc).ratio() >= fuzzy_threshold:
                            keep_ok = True
                            break

            if not keep_ok and keep_re:
                if any(p.search(raw) for p in keep_re):
                    keep_ok = True

            if keep_ok:
                kept.append(raw.strip())

        if not kept and fallback_to_all_on_empty:
            return (text if text.endswith("\n") else text + "\n")

        return ("\n".join(kept) + ("\n" if kept else ""))


    def test_cases_filtered(
        self,
        binary,
        in_filenames,
        folder="testes",
        ...
        *,
        ...
        for in_filename in in_filenames:
            ...
            res_filename = in_filename.replace(".in", ".res")
            try:
                with open(in_filename) as i:
                    p = self.run_binary(cmd, i, PIPE, in_filename=in_filename)
                output = p.stdout or ""
                output = self._filter_output(
                    ...
                    drop_regexes=drop_regexes,
                    fuzzy_keep=fuzzy_keep,
                    fuzzy_threshold=fuzzy_threshold,
                    fallback_to_all_on_empty=fallback_to_all_on_empty,
                )
                with open(out_filename, "w", encoding="utf-8", errors="ignore") as o:
                    o.write(output)
                self.tested_cases.add(in_filename)
                ok = self.compare_files(out_filename, res_filename)
                assert ok, f'execute: diff "{out_filename}" "{res_filename}"'
...
class Task(BaseTask):
    def teste_1_ciclos(self):
        cases = self.case_range("ciclo{}.in", 1, 3)
        binary = self.make("buildmap")
        self.test_cases_filtered(
            binary,
            cases,
            folder="testes",
            keep_contains=["Erro:", "Ciclo:"],
            fuzzy_keep=False,
            fallback_to_all_on_empty=True,
        )

    def teste_2_dfs(self):
        cases = self.case_range("dfs{}.in", 1, 4)
        binary = self.make("buildmap")
        self.test_cases_filtered(
            binary,
            cases,
    ...
    def teste_3_bfs(self):
        cases = self.case_range("bfs{}.in", 1, 4)
        binary = self.make("buildmap")
        self.test_cases_filtered(
            binary,
            cases,
            folder="testes",
            keep_contains=["distancia ate"],
            fuzzy_keep=False,
            fallback_to_all_on_empty=True,
        )

    def teste_4_pequenos(self):
        cases = self.case_range("pequeno{}.in", 1, 5)
        binary = self.make("buildmap")
        self.test_cases_filtered(
            binary,
            cases,
            folder="testes",
            keep_contains=["SEQ", "Erro:", "Ciclo:", "distancia ate"],
            fuzzy_keep=False,
            fallback_to_all_on_empty=True,
        )

    def teste_5_medios(self):
