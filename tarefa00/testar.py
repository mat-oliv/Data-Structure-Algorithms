...
class Task(BaseTask):
    def teste_1_bomdia(self):
        cases = self.case_range("bomdia.in", 1, 1)
        binary = self.compile(["bomdia.c"])
        self.input_output(binary, "Antônio", "Bom dia, Antônio.")
        self.test_cases(binary, cases)

    def teste_2_boanoite(self):
        cases = self.case_range("boanoite{}.in", 1, 2)
        binary = self.make("boanoite")
        self.test_cases(binary, cases)
