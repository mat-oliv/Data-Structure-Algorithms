...
int main(){
  ...
  if(print_order->g == -2){
    ...
    for(int x = 0; x < print_order->h/2; x++){
      // print_order aqui guarda o tamamnho do ciclo, que é print_order->h
      // Essa função aqui inverte o ciclo, por que Se a recursão guardar: A B C D, então o caminho percorrido foi D C A B
   ...
   for(int x = 1; x < print_order->h; x++){
    if(strcmp(print_order->print_order[x]->name, max) < 0) { // Aqui eu pego que tem prioridade de valor lexicográfico na lista.
  ...
  print_order->h -= 1;
    ...
    h += 1;
    while (1){
      // Aqui eu vou printar todos os valores de print_order em ciclo, começando do prioridade de lexicográfico até voltar nele de novo.
      if (h >= print_order->h) { 
        if(first_h != 0)
        h = 0;
        else{
